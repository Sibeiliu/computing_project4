...
assign(p[8],ggplot(data = chain, aes_string(x = "index", y = paste("V",i,sep=""))) + geom_point(color = rgb(.2, .4, .2, .5)) +
    geom_line() + 
    ggtitle("Sigma") + theme(plot.title = element_text(hjust = 0.5)))

assign(p[9],ggplot(data = chain, aes_string(x = "index", y = paste("V",i,sep=""))) + geom_point(color = rgb(.2, .4, .2, .5)) +
    geom_line() + 
...
chain_new = chain[15000:20000,]

q <- c(paste0('plot_final',1:9))
  for(i in 1:7){
  assign(q[i],ggplot(data = chain_new, aes_string(x = "index", y = paste("V",i,sep=""))) + geom_point(color = rgb(.2, .4, .2, .5)) +
    geom_line() + 
    ggtitle(paste("Beta",i-1,sep="")) + theme(plot.title = element_text(hjust = 0.5)))
  }

assign(q[8],ggplot(data = chain_new, aes_string(x = "index", y = paste("V",i,sep=""))) + geom_point(color = rgb(.2, .4, .2, .5)) +
    geom_line() + 
    ggtitle("Sigma") + theme(plot.title = element_text(hjust = 0.5)))

assign(q[9],ggplot(data = chain_new, aes_string(x = "index", y = paste("V",i,sep=""))) + geom_point(color = rgb(.2, .4, .2, .5)) +
    geom_line() + 
    ggtitle("Rho") + theme(plot.title = element_text(hjust = 0.5)))
